...
// Feedback runtime mounted at app level.
// Existing App content remains unchanged.
...
export default function App() {
  ...
  return <>
    ...
    <main className="app-shell">
      ...
      {!runtime && <section className="chapter-card">{asyncBoss.phases.flatMap((phase) => phase.questIds).map((questId)=><button key={questId} onClick={()=>startQuest(questId)}>{questId}</button>)}{<button onClick={startBoss}>进入 Boss 战</button>}</section>}
      {runtime && activeQuest && <section className="challenge-card"><ChallengeContent challenge={activeQuest.challenge} selectedAnswer={runtime.selectedAnswer} onSelect={selectAnswer}/><button onClick={submitAnswer}>提交答案</button><button onClick={runtime.result?.passed ? exitQuest : retryQuest}>继续</button><button onClick={useHint}>提示</button><button onClick={startBossPhase}>Boss阶段</button></section>}
    </main>
  </>;
}
